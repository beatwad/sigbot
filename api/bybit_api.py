...
cryptocurrency exchange API. It defines the `ByBit` class, which
...
class ByBit(ApiBase):
    """Class for accessing ByBit cryptocurrency exchange API"""
    ...
    def connect_to_api(self, api_key: str, api_secret: str) -> unified_trading.HTTP:
        """
        ...
        """
        client = unified_trading.HTTP(api_key=api_key, api_secret=api_secret)
        return client
    ...
    def get_ticker_names(self, min_volume: float) -> Tuple[List[str], List[float], List[str]]:
        ...
        tickers = tickers[
            (tickers["symbol"].str.endswith("USDT")) | (tickers["symbol"].str.endswith("USDC"))
        ]
    ...
    def get_klines(self, symbol: str, interval: str, limit: int) -> pd.DataFrame:
        ...
        tickers = pd.DataFrame(
            self.client.get_kline(category="spot", symbol=symbol, interval=interval, limit=limit)[
                "result"
            ]["list"]
        )
