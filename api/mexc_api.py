...
class MEXC(ApiBase):
    ...
    def get_klines(self, symbol: str, interval: str, limit: int = 300) -> pd.DataFrame:
        ...
        tickers = pd.DataFrame(requests.get(self.URL + "/klines", params=params, timeout=3).json())
    ...
    def get_historical_klines(
        ...
        while earliest_time > min_time:
            ...
            tmp = pd.DataFrame(requests.get(self.URL + "/klines", params=params, timeout=3).json())
            if tmp.shape[0] == 0:
                break
