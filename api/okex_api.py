...
class OKEX(ApiBase):
    ...
    def get_ticker_names(self, min_volume) -> Tuple[List[str], List[float], List[str]]:
        ...
        tickers = pd.DataFrame(
            requests.get(self.URL + "/api/v5/market/tickers?instType=SPOT", timeout=3).json()[
                "data"
            ]
        )
        ...
        tickers = tickers[
            (tickers["instId"].str.endswith("USDT")) | (tickers["instId"].str.endswith("USDC"))
        ]
    ...
    def get_klines(self, symbol: str, interval: str, limit: int = 200) -> pd.DataFrame:
        ...
        tickers = pd.DataFrame(
            requests.get(self.URL + "/api/v5/market/candles", params=params, timeout=3).json()[
                "data"
            ]
        )
