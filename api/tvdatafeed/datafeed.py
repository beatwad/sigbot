...
class TvDatafeedLive(tvDatafeed.TvDatafeed):
    ...
    class _SeisesAndTrigger(dict):
        ...
        def wait(self):
            ...
            if (
                not self._trigger_quit
            ):  # if not quitting then we can clear interrupt before sarting the wait
            ...
            while (
                True
            ):  # might need to restart waiting if trigger_dt changes and interrupted when waiting
                ...
                ) and self._trigger_quit:  # if we received a shutdown event during waiting
                    return False
        ...
        def get_expired(self):
            ...
            for interval, values in self.items():
                if dt.now() >= values[1]:
                    ...
                    values[1] = (
                        values[1] + self._timeframes[interval]
                    )  # add interval to get new expiry dt in future
        ...
        def append(self, seis, update_dt=None):
            ...
            if seis.interval.value in self.keys():  # interval group already exists
                ...
                if update_dt is None:
                    ...
                    update_dt = (
                        update_dt + self._timeframes[seis.interval.value]
                    )  # change the time to next update datetime (result will be datetime object)
                    self.__setitem__(seis.interval.value, [[seis], update_dt])

                    if (
                        trigger_dt := self._next_trigger_dt()
                    ) != self._trigger_dt:  # if new interval group expiry is sooner than current expiry being waited on
        ...
        def discard(self, seis):
            ...
            if seis not in self:
                ...
                if not super().__getitem__(seis.interval.value)[
                    0
                ]:  # if interval group now empty then remove it
                    self.pop(seis.interval.value)

                    if ((trigger_dt := self._next_trigger_dt()) != self._trigger_dt) and (
                        self._trigger_quit is False
                    ):  # if interval group expiry dt was being waited on and havent quit
                        self._trigger_dt = trigger_dt
                        self._trigger_interrupt.set()
    ...
    def new_seis(self, symbol, exchange, interval, timeout=-1):
        ...
        if self._args_invalid(symbol, exchange):
            raise ValueError(
                "Provided symbol and exchange combination is not listed in TradingView"
            )

        if seis := self._sat.get_seis(
            symbol, exchange, interval
        ):  # if Seis with such parameters already exists then simply return that
            return seis
        ...
        if interval_key not in self._sat.intervals():
            ...
            )  # get ticker data bar for this symbol from TradingView
            update_dt = ticker_data.index.to_pydatetime()[
                0
            ]  # extract datetime of when this bar was produced/released
    ...
    def _main_loop(self):
        ...
        while (
            self._sat.wait()
        ):  # waits until soonest expiry and returns True; returns False if closed
            with self._lock:
                for (
                    interval
                ) in self._sat.get_expired():  # returns a list of intervals that have expired
                    for seis in self._sat[
                        interval
                    ]:  # go through all the seises in this interval group
                        for _ in range(0, RETRY_LIMIT):  # re-try maximum of RETRY_LIMIT times
                            data = super().get_hist(
                                ...
                                n_bars=2,
                            )  # get_hist returns bars starting with currently open so need to read 2 to get first closed
                            if data is not None:  # check that we did get any data
                                if seis.is_new_data(data):  # check that it is new data not old
                                    data = data.drop(
                                        labels=data.index[1]
                                    )  # drop the row (last) which has yet un-closed bar data
                                    break
