...
def model_train(
    ...
    if train_test == "fold" and loop == "outer":
    ...
    elif train_test == "fold" and loop == "inner":
        oof = np.zeros([len(df), 1])
        ...
        if verbose:
            plt.ylim(0.5, n_folds * 3 + 0.5)
            plt.xlabel("Index")
            plt.ylabel("Fold")
            plt.title("Train/Test Distribution for Nested Time-Series Split")
            plt.legend(["Train", "Test"], loc="lower right")
            plt.show()
