...
class ByBitPerpetual(ApiBase):
    ...
    def get_ticker_names(
        ...
        tickers = pd.DataFrame(
            self.client.get_tickers(category="linear")["result"]["list"]
        )
        ...
        while earliest_time > min_time:
            ...
            if funding_rates.shape[0] > 0:  # drop duplicates
                funding_rates = funding_rates[
                    funding_rates["fundingRateTimestamp"]
                    > tmp["fundingRateTimestamp"].max()
                ]
    ...
    def find_open_orders(self) -> None:
        ...
        orders = self.client.get_open_orders(
            category="linear", settleCoin=self.quote_coin
        )["result"]["list"]
        for order in orders:
            symbol = order["symbol"]
    ...
    def _get_last_similar_trade(
        ...
        trade_history = [
            t_h
            for t_h in trade_history
            if t_h["side"] == side and t_h["orderQty"] == size
        ]
        if trade_history:
    ...
    @staticmethod
    def _set_trigger_price(
        price: float, direction: str, tick_size: float
    ) -> Tuple[int, float]:
        """
