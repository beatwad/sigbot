...
class MEXCFutures(ApiBase):
    ...
    def get_ticker_names(
        ...
        tickers = pd.DataFrame(
            requests.get(self.URL + "/ticker", timeout=3).json()["data"]
        )
    ...
    def get_klines(self, symbol: str, interval: str, limit: int = 300) -> pd.DataFrame:
        ...
        tickers = pd.DataFrame(
            requests.get(
                self.URL + f"/kline/{symbol}", params=params, timeout=3
            ).json()["data"]
        )
    ...
    def get_historical_funding_rate(
        ...
        while earliest_time > min_time:
            tmp = pd.DataFrame(
                requests.get(
                    self.URL + "/funding_rate/history", params=params, timeout=3
            ...
            if funding_rates.shape[0] > 0:
                funding_rates = funding_rates[
                    funding_rates["settleTime"] > tmp["settleTime"].max()
                ]
