...
class SigBot:
    ...
    def __init__(
        ...
        **configs,
        ...
        self.higher_tf_indicator_set = {
            i for i in self.higher_tf_indicator_list if i != "Trend"
        }
        # List of Futures Exchanges
        self.futures_exchanges = configs["Exchanges"]["futures_exchanges"]
        # Create indicators
        self.higher_tf_indicators, self.work_tf_indicators = self._create_indicators(
            configs
        )
        ...
        self.max_prev_candle_limit = configs["Signal_params"]["params"][
            "max_prev_candle_limit"
        ]
    ...
    def get_api_and_tickers(self) -> None:
        ...
        for i, ex in enumerate(exchange_list):
            ...
            # get ticker list
            try:
                tickers, ticker_vols, all_tickers = self.exchanges[ex][
                    "API"
                ].get_tickers()
    ...
    def _filter_used_tickers(
        self, tickers: list, ticker_vols: list
    ) -> Tuple[list, list]:
        """
    ...
    def add_indicators(
        ...
        if (
            data_qty > 1
            and self.main.cycle_number > 1
            and timeframe == self.work_timeframe
        ):
    ...
    def filter_old_signals(self, sig_points: list) -> list:
        ...
        for point in sig_points:
            ...
            if indicator_list.intersection(self.higher_tf_indicator_set):
                ...
                time_span = (
                    self.timeframe_div[self.work_timeframe] * self.max_prev_candle_limit
                )
    ...
    def calc_statistics(self, sig_points: list) -> list:
        ...
        for sig_point in sig_points:
            ...
            pattern = sig_point[5]
            result_statistics, _ = self.stat.calculate_total_stat(
                self.database, sig_type, pattern
            )
    ...
    def save_opt_dataframes(
        ...
        """
        self.spot_ex_monitor_list, self.fut_ex_monitor_list = (
            self._create_exchange_monitors()
        )
        dt_now = datetime.now()
        if load:
            print("\nLoad the datasets...")
        ...
        for ex in self.spot_ex_monitor_list:
            if ex.exchange_data["API"].name == "Binance":
                ...
                if btcd.shape[0] > 0 or (btcd.shape[0] == 0 and self.btcd is None):
                    self.btcd = btcd
                if btcdom.shape[0] > 0 or (
                    btcdom.shape[0] == 0 and self.btcdom is None
                ):
                    self.btcdom = btcdom
        # start all futures exchange monitors
        for monitor in self.fut_ex_monitor_list:
...
class MonitorExchange:
    ...
    def mon_add_statistics(
        ...
        data_qty_higher
            ...
            to which indicators will be added.
        """
        self.sigbot.database = self.sigbot.sb_add_statistics(
            sig_points, data_qty_higher
        )
            ...
            for timeframe in self.sigbot.timeframes:
                if historical:
                    ...
                    df, data_qty = self.sigbot.get_new_data(
                        exchange_api, ticker, timeframe, dt_now
                    )
                # If we previously download this dataframe to the disk -
                # update it with new data
                if data_qty > 1:
                    tmp_ticker = self.sigbot.delete_redundant_symbols_from_ticker(
                        ticker
                    )
                    try:
                        tmp = pd.read_pickle(  # nosec
                            f"../optimizer/ticker_dataframes/"
                            f"{tmp_ticker}_{timeframe}.pkl"
                        )
                    ...
                    df_path = (
                        f"../optimizer/ticker_dataframes/{tmp_ticker}_{timeframe}.pkl"
                    )
    ...
    def mon_save_opt_statistics(
        self, ttype: str, opt_limit: int, opt_flag: bool
    ) -> None:
        """
    ...
    def run_cycle(self) -> None:
        ...
        processes = []
        print(
            f"Exchange: {self.exchange}, number of tickers: {len(tickers)}", flush=True
        )
        for ticker in tickers:
            data_qty_higher = 0
            ...
            for timeframe in self.sigbot.timeframes:
                ...
                if data_qty > 1:
                    ...
                    if timeframe == self.sigbot.work_timeframe:
                        ...
                        if (
                            self.sigbot.main.cycle_number
                            > self.sigbot.main.first_cycle_qty_miss
                        ):
                            ...
                            sig_points = self.sigbot.get_exchange_list(
                                ticker, sig_points
                            )
                            ...
                            if sig_points:
                                ...
                                sig_points = self.sigbot.make_prediction(
                                    sig_points, self.exchange
                                )
