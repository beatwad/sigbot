"""
This module provides a framework for retrieving and processing cryptocurrency
candlestick data from various exchanges, including Binance, ByBit, OKEX, MEXC,
and their futures markets.
It includes functionality for handling spot and futures data, as well as tools to
retrieve historical price information, candlestick data, and funding rates.
"""
...
class GetData:
    ...
    def __init__(self, **configs):
        ...
        **configs : dict
        ...
        self.timeframe_div = self.configs.get("timeframe_div", {})
    ...
    def load_saved_data(df: pd.DataFrame, ticker: str, timeframe: str) -> pd.DataFrame:
        ...
        try:
            df = pd.read_pickle(  # nosec
                f"optimizer/ticker_dataframes/{ticker}_{timeframe}.pkl"
            )
    ...
    def get_data(
        ...
        for i in range(self.num_retries):
            try:
                klines = self.api.get_klines(
                    ticker, timeframe, min(limit + 2, self.limit)
                )
            except BaseException:  # noqa
                if i == self.num_retries - 1:
                    logger.exception(
                        f"Catch an exception while trying to get candles. "
        ...
        if timeframe == self.work_timeframe:
            ...
            for i in range(self.num_retries):
                try:
                    funding_rates = self.api.get_historical_funding_rate(
                        ticker, min(limit + 2, self.limit // 2), min_time
                    )
                except BaseException:  # noqa
                    if i == self.num_retries - 1:
                        logger.exception(
                            f"Catch an exception while trying to get funding rate. "
                            f"API is {self.api}, ticker is {ticker}"
                        )
                    sleep(1)
                    continue
    ...
    def get_btc_dom(self) -> Tuple[pd.DataFrame, pd.DataFrame]:
        ...
        for i in range(self.num_retries):
            try:
                ...
                if i == self.num_retries - 1:
                    logger.exception(
                        "Catch an exception while trying to get BTC dominance."
                    )
                sleep(1)
                continue
            else:
        ...
        for i in range(self.num_retries):
            try:
                klines = self.api.get_historical_klines(
                    ticker, timeframe, self.limit, min_time
                )
                funding_rates = self.api.get_historical_funding_rate(
                    ticker, self.limit, min_time
                )
            except BaseException:  # noqa
                if i == self.num_retries - 1:
                    logger.exception(
    ...
    def add_funding_rate(
        self, df: pd.DataFrame, funding_rates: pd.DataFrame, timeframe: str
    ) -> pd.DataFrame:
        """
        ...
        if timeframe == self.work_timeframe:
            if funding_rates.shape[0] > 0:
                ...
                if (
                    self.name == "OKEXSwap"
                ):  # OKEXSwap funding rate history is capped with 3 months
                    df["funding_rate"] = df["funding_rate"].fillna(0)
            else:
                df["funding_rate"] = 0
        return df

    def get_hist_funding_rate_data(
        self, ticker: str, min_time: datetime
    ) -> tuple[pd.DataFrame, int]:
        """
        ...
        -------
        tuple[pd.DataFrame, int]
            A tuple containing the dataframe with funding rate data and
            the retrieval limit.
        """
        for i in range(self.num_retries):
            try:
    ...
    def fill_ticker_dict(self, tickers: str) -> None:
        """
        Initialize the ticker dictionary with the current timestamp for each
    ...
    def process_data(self, klines: pd.DataFrame, df: pd.DataFrame) -> pd.DataFrame:
        ...
        klines[["open", "high", "low", "close", "volume"]] = klines[
        ...
        if df.shape[0] == 0:
            ...
            df = df.iloc[max(df.shape[0] - self.limit, 0) :].reset_index(  # noqa
                drop=True
            )
    ...
    def process_funding_rate_data(self, funding_rates) -> pd.DataFrame:
        ...
        ----------
        funding_rates : pd.DataFrame
            Dataframe containing funding rate data with columns such as
            'funding_rate' and 'time'.
    ...
    def add_indicator_data(
        ...
        """
        Add indicator data to the cryptocurrency dataframe and update the dataframe
        dictionary.

        This method processes a list of indicators, applying them to the dataframe,
        and updates the dictionary of dataframes with the new indicator data.

        Parameters
        ----------
        dfs : dict
            Dictionary to store dataframes for different tickers, timeframes and
            trade types.
        df : pd.DataFrame
        ...
        if timeframe not in dfs[ticker]:
            dfs[ticker][timeframe] = {}
        if "data" not in dfs[ticker][timeframe]:
            dfs[ticker][timeframe]["data"] = {}
    ...
    def get_time_label(dt_now: datetime, timeframe: str) -> int:
        ...
        -------
        int
            The time label for the given timeframe, representing which part of the
            time interval we are in.
        """
        if timeframe == "5m":
    ...
    def get_limit(
        ...
        """
        Determine the limit for how much data to retrieve based on the
        existing data and the current time.
        ...
        if df.shape[0] == 0:
            ...
            return self.limit
        # if enough time has passed and time label has changed -
        # increase the limit to update candle data
