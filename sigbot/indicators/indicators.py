...
class RSI(Indicator):
    ...
    def get_indicator(
        self, df, ticker: str, timeframe: str, data_qty: int, *args
    ) -> pd.DataFrame:
        """
...
class PumpDump(Indicator):
    ...
    def get_price_stat(self) -> None:
        ...
        try:
            ...
            for key in self.price_stat.keys():
                ...
                self.price_stat[key] = {
                    float(k): int(v) for k, v in self.price_stat[key].items()
                }
    ...
    def get_price_change(self, df: pd.DataFrame, data_qty: int, lag: int) -> list:
        ...
        return df[f"price_change_{lag}"][
            max(df.shape[0] - data_qty + 1, 0) :  # noqa
        ].values
        ...
        for i in range(1, 2):
            ...
            if len(self.price_stat[f"lag{i}"]) < self.max_stat_size:
                self.price_stat[f"lag{i}"] = np.append(
                    self.price_stat[f"lag{i}"], close_prices
                )
                # delete NaNs
            ...
            q_low_lag = np.quantile(
                self.price_stat[f"lag{i}"], self.low_price_quantile / 1000
            )
            q_high_lag = np.quantile(
                self.price_stat[f"lag{i}"], self.high_price_quantile / 1000
            )
