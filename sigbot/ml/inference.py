...
class Model:
    ...
    def prepare_data(
        ...
        tmp_df[btcd_cols] = pd.merge(
            tmp_df[["time"]], btcd[btcd_cols], how="left", on="time"
        )
        tmp_df[btcdom_cols] = pd.merge(
            tmp_df[["time"]], btcdom[btcdom_cols], how="left", on="time"
        )
        ...
        for i, point in enumerate(signal_points):
            ...
            for key, features in self.feature_dict.items():
                if key.isdigit():
                    try:
                        tmp_row = tmp_df.loc[
                            tmp_df["time"]
                            == point_time - pd.to_timedelta(int(key), unit="h"),
                            features,
            ...
            if row.isnull().sum().sum() > 0:
                logger.info(
                    f"Ticker {ticker} signal with time "
