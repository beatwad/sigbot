...
class TelegramBot:
    ...
    def __init__(
        ...
        **configs,
        ...
        self.max_notifications_in_row = self.configs.get(
            "self.max_notifications_in_row", 3
        )
    ...
    def send_notification(self, message: list) -> None:
        ...
        message_thread_id = self.message_thread_ids.get(
        ...
        cleaned_sig_pattern = (
            sig_pattern[:-9] if sig_pattern.endswith("Volume24") else sig_pattern
        )
        # create message
        ...
        if sig_img_path:
            ...
            if (
                ...
                self.send_photo(
                    favorite_chat_id, favorite_message_thread_id, sig_img_path, text
                )
    ...
    def send_message(self, chat_id: str, message_thread_id: int, text: str):
        ...
        tasks = [
            self.loop.create_task(
                self.bot_send_message(self.bot, chat_id, message_thread_id, text)
            )
        ]
    ...
    @exception
    def send_photo(
        self, chat_id: str, message_thread_id: int, img_path: str, text: str
    ):
        """
        ...
        tasks = [
            self.loop.create_task(
                self.bot_send_photo(
                    self.bot, chat_id, message_thread_id, img_path, text
                )
            )
        ]
        self.loop.run_until_complete(asyncio.wait(tasks))
