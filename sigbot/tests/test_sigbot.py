...
def sigbot(mocker):
    ...
    sigbot.exchanges = {'Binance': {'API': None, 'tickers': [], 'all_tickers': []},
    ...
    sigbot.database = {
        ticker: {
            '5m': {'data': {ttype: pd.DataFrame(
                {
                    'time': [pd.to_datetime('2024-09-24 10:00')]
                }
            )}},
            '1h': {'data': {ttype: pd.DataFrame(
                {
...
def test_filter_used_tickers(sigbot, expected):
    ...
    assert sigbot.filter_used_tickers(tickers, prev_tickers) == (expected[0], expected[1])
...
def test_add_indicators(sigbot):
    ...
    database, returned_data_qty = sigbot.add_indicators(df, 'buy', ticker, timeframe, exchange_data, data_qty, opt_flag)
...
def test_get_buy_signals(sigbot):
    ...
    sigbot.find_signal_buy.find_signal.assert_called_once_with(sigbot.database, ticker, timeframe, 10, 5)
...
def test_get_sell_signals(sigbot):
    ...
    sigbot.find_signal_sell.find_signal.assert_called_once_with(sigbot.database, ticker, timeframe, 10, 5)
