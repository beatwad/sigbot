...
class SignalStat:
    ...
    **configs : dict
    ...
    def calculate_total_stat(self, dfs: dict, ttype: str, pattern: str) -> Tuple[list, int]:
        """
        ...
        for t in range(1, self.stat_range + 1):
            ...
            pct_price_diff_mean = round(
                stat.loc[stat[f"pct_price_diff_{t}"] != 0, f"pct_price_diff_{t}"].mean(),
                2,
            )
    ...
    def check_close_trades(
        ...
        same_signal = stat[
            (stat["ticker"] == ticker) & (stat["pattern"] == pattern) & (stat["time"] == point_time)
        ]
        ...
        if set(pattern.split("_")).intersection(self.higher_tf_indicator_set):
            if (point_time - last_signal_timestamp).total_seconds() > self.timeframe_div[
