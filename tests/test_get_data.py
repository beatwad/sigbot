...
@pytest.mark.parametrize(
    "exchange, df, ticker, timeframe, expected",
    [
        (
            "Binance",
            pd.DataFrame(columns=df_btc_5m.columns),
            "BTCUSDT",
            "5m",
...
cryptocurrencies1 = (
    df_btc_5m[["time", "open", "high", "low", "close", "volume"]]
    .loc[498:499]
...
cryptocurrencies5 = (
    df_eth_5m[["time", "open", "high", "low", "close", "volume"]]
    .loc[400:599]
    .reset_index(drop=True)
)
cryptocurrencies6 = (
    df_eth_1h[["time", "open", "high", "low", "close", "volume"]]
    .loc[400:599]
    .reset_index(drop=True)
)
...
@pytest.mark.parametrize(
    "df, index, limit, expected",
    [
        (pd.DataFrame(), 0, 500, df.loc[498:499].reset_index(drop=True)),
...
@pytest.mark.parametrize(
    "df, ticker, timeframe, index, limit, expected",
    [
        (
            pd.DataFrame(),
            "BTCUSDT",
        ...
        (
            ...
            1,
            2,
            (df.loc[2:500].reset_index(drop=True), 2),
        ),
        (
            df.loc[:499],
            "BTCUSDT",
            ...
            2,
            100,
            (df.loc[100:598].reset_index(drop=True), 100),
        ),
        (
            df.loc[:499],
            "BTCUSDT",
            ...
            3,
            200,
            (df.loc[100:598].reset_index(drop=True), 200),
        ),
    ],
    ids=repr,
)
def test_get_data(mocker, df, ticker, timeframe, index, limit, expected):
    tmp = cryptocurrencies[index].copy()
    mocker.patch("api.binance_api.Binance.connect_to_api", return_value=None)
...
def test_add_indicator_data(mocker, df, ticker, timeframe, expected):
    ...
    data_qty = 20
    res = gd.add_indicator_data(dfs, data, "buy", indicators, ticker, timeframe, data_qty)[ticker][
        timeframe
    ]["data"]["buy"]
