...
df_ind1 = pd.read_pickle('test_BTCUSDT_1h_indicators.pkl')
df_ind2 = pd.read_pickle('test_BTCUSDT_5m_indicators.pkl')

level1 = [[23675.36, 2], [22935.97, 2], [22031.31, 2], [22589.28, 2], [22257.15, 1], [21808.01, 2], [20856.48, 2],
         [21196.58, 2], [23196.09, 2], [23988.32, 2], [24442.66, 1], [23414.03, 2], [21385.04, 2], [21548.71, 2]]
level2 = [[21382.43, 3], [21467.33, 2], [21571.43, 3], [21665.0, 2], [21286.96, 2], [20901.22, 3], [21217.95, 3],
          [21097.1, 2],  [20979.69, 1], [21153.7, 3], [[23675.36, 2], 1], [[22935.97, 2], 1], [[22031.31, 2], 1],
          [[22589.28, 2], 1], [[22257.15, 1], 1], [[21808.01, 2], 1], [[23196.09, 2], 1], [[23988.32, 2], 1],
          [[24442.66, 1], 1], [[23414.03, 2], 1]]

df_inds = [df_ind1, df_ind2]
levels = [level1, level2]

@pytest.mark.parametrize('dfs, df, ticker, timeframe, expected',
                         [
                          (dfss, df_btc_1, 'BTCUSDT', '1h', (df_inds[0], levels[0])),
                          (dfss, df_btc_5, 'BTCUSDT', '5m', (df_inds[1], levels[1])),
                          # (dfss[1], df_eth_5, 'ETHUSDT', '5m', None),
                          # (dfss[1], df_eth_5, 'ETHUSDT', '1h', None),
                          ], ids=repr)
def test_add_indicator_data(mocker, dfs, df, ticker, timeframe, expected):
    indicators = list()
    ...
    mocker.patch('api.binance_api.Binance.get_crypto_currency', return_value=cryptocurrencies[3])
    gd = DataFactory.factory('Binance', **configs)
