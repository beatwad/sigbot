...
point1 = [
    ...
    816,
    "sell",
    pd.to_datetime("2022-09-27 07:05:00"),
...
point2 = [
    ...
    966,
    "buy",
    pd.to_datetime("2022-09-27 19:35:00"),
...
point3 = [
    ...
    980,
    "buy",
    pd.to_datetime("2022-09-27 20:45:00"),
    [("PumpDump", ()), ("Trend", ())],
    [],
    ["Binance", "BinanceFutures"],
    [
...
@pytest.mark.parametrize(
    "point, expected",
    [(points[0], keys[0]), (points[1], keys[1]), (points[2], keys[2])],
    ids=repr,
)
...
def test_get_prev_mean_pct_right_forecast(mocker, point, prev_stat_dict, expected):
    mocker.patch("builtins.open", mocker.mock_open(read_data=None))
    mocker.patch("json.load", return_value=prev_stat_dict)
...
stat_results = [
    ...
    "= increased on / вырос на 100.0",
    "= decreased on / уменьшилcя на 10.0",
]


@pytest.mark.parametrize(
    "prev_mean_pct_right_forecast, mean_pct_right_forecast, expected",
    [
        (
            prev_mean_pct_right_forecasts[0],
            mean_pct_right_forecasts[0],
            stat_results[0],
        ),
        (
            prev_mean_pct_right_forecasts[1],
            mean_pct_right_forecasts[1],
            stat_results[1],
        ),
        (
            prev_mean_pct_right_forecasts[2],
            mean_pct_right_forecasts[2],
            stat_results[2],
        ),
    ],
    ids=repr,
