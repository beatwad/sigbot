...
class Visualizer:
    ...
    def save_plot(self, ticker, timeframe, pattern, data):
        ...
        """
        filename = (
            f"{self.image_path}/{ticker}_{timeframe}_" f"{pattern}_{data['time'].iloc[-1]}.png"
        )
    ...
    def create_plot(self, dfs: dict, point: list, levels: list):  # pylint: disable=R0912, R0915
        """
        Create the plot
        ...
        if "Pattern" in indicator_list_tmp:
            ...
            ohlc_higher = df_higher[["time", "open", "high", "low", "close", "volume"]].set_index(
                "time"
            )
            ...
            if indicator_tmp in ("Trend", "MACD"):
                axs_higher[0].set_title(
                    f"{self.process_ticker(ticker)} - " f"{self.higher_timeframe} - Trend / Тренд",
                    fontsize=14,
                    color=self.ticker_color,
                )
